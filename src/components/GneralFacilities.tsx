...
import imagegolf from '../assets/Image (23).png'
import {
  ChevronDown,
  ChevronLeft,
  ChevronRight,
...
export default function GeneralFacilities() {
  ...
  return (
    <div className=""> <section className="bg-white md:bg-[#f6f5f1] px-5 py-6 md:py-12 md:px-16">
      <div className="max-w-md md:max-w-none mx-auto md:mx-0">
        {/* Header */}
        <header className="mb-4 md:mb-10 f justify-between">
          <h1 className="text-2xl md:text-6xl font-bold md:font-normal md:font-serif text-slate-800 md:text-neutral-900 text-center">
            General facilities
          </h1>
          <p className="text-center"> Facilities and amenities available in all types of clusters<br />, from Type-A to Type-C</p>
            ...
            <ul className="absolute z-10 mt-1 w-full bg-white border border-neutral-200 rounded-xl shadow-lg overflow-hidden">
              {amenities.map((a, i) => (
                <li key={a.id}>
                  <button
                    onClick={() => select(i)}
                    className={`w-full text-left px-4 py-3 text-sm transition-colors ${i === activeIndex
              ...
              <div className="flex gap-1.5 md:hidden">
                {amenities.map((a, i) => (
                  <button
                    ...
                    className={`h-1 flex-1 rounded-full transition-colors ${i === activeIndex ? "bg-white" : "bg-white/30"
                      }`}
                  />
                ))}
              </div>
            ...
            {amenities.map((a, i) => {
              ...
              return (
                <li key={a.id}>
                  <button
                    onClick={() => select(i)}
                    className={`w-full flex items-baseline justify-between py-5 md:py-6 text-left transition-colors ${isActive
                        ? "text-neutral-900"
                        : "text-neutral-500 hover:text-neutral-800"
                      }`}
                  >
                    <span
                      className={`font-serif tracking-tight transition-all ${isActive
                    ...
                    <span
                      className={`text-xs tabular-nums ${isActive ? "text-[#2e4a34]" : "text-neutral-400"
                        }`}
                    >
                      {a.order}
                    </span>
